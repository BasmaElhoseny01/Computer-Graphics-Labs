...
struct Vertex
{
    ...
    Color color;
    glm::vec2 tex_coord; // Texture coordinate for the vertex ie this vertex what is the corresponding position in the texture
};
...
int main()
{
    ...
    Vertex positions[4] = {// This is now on the CPU RAM
                           {{0.5, 0.5, 0.0}, {0, 255, 0, 255}, {1.0, 1.0}},
                           {{0.5, -0.5, 0.0}, {0, 255, 255, 255}, {1.0, 0.0}},  // common
                           {{-0.5, 0.5, 0.0}, {255, 255, 0, 255}, {-1.0, 1.0}}, // common
    ...
    // Lab(4):Texture  Step(1)
    Color B = {0, 0, 0, 255};
    Color W = {255, 255, 255, 255};
    Color Y = {255, 255, 0, 255};
    // We here created an array of pixels [in Ram ] to save on GPU ==> use Texture
    Color image[] = {
        W,
        W,
        Y,
    ...
    GLuint texture;
    glGenTextures(1, &texture);            // creating 1 texture
    glBindTexture(GL_TEXTURE_2D, texture); // binding 2d Tex
    // 2D Texture- -Format to save the data(GL_RGBA8)-> 8 bits for each channel RGBA - width -height- border (This value must be 0.)
    // format:Specifies the format of the pixel data. RGBA since struct Color{uint8_t r, g, b, a; // 1 byte 8bits};
    // type:Specifies the data type of the pixel data.  each element of the Color stuct is unsigned byte
    // dataSpecifies a pointer to the image data in memory[RAM].
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, 8, 8, 0, GL_RGBA, GL_UNSIGNED_BYTE, image); // sending data the texture
    // Till here we had saved the texture in the GPU how to use it ??
    glGenerateMipmap(GL_TEXTURE_2D);

    // For pixel Art without interpolation
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Lab(4):Texture  Step(3)
        // 1.Activate first locker in my desk :D we have 32 locker here
        glActiveTexture(GL_TEXTURE0);          // Open first locker
        glBindTexture(GL_TEXTURE_2D, texture); // 2.Added data in the first locker
        glUniform1i(tex_loc, 0);               // 3.Tell Shader that your data is in locker 0
